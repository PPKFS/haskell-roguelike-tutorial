...
import Rogue.FieldOfView.Visibility ( Viewshed(visibleTiles, range) )
...
moveActorInDirection actor dir = do
  o <- getActor (actorID actor)
  let newPos = calculateNewLocation dir $ o ^. objectPosition
  modifyObject o (objectPosition .~ newPos)
  makeViewshedDirty (actorID actor)
